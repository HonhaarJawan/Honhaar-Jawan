...
const EmailTemplateList = () => {
    ...
    try {
      ...
      for (const result of templatesToProcess) {
        ...
        if (activeTab === "single") {
          ...
          // Replace in content
          if (findReplaceOptions.searchInContent && template.template) {
            const newContent = template.template.replace(
              searchRegex,
              replaceText
            );
          ...
          // Replace in subject - FIXED: Remove the unnecessary includes check
          if (findReplaceOptions.searchInSubject && template.subject) {
          ...
          if (findReplaceOptions.searchInContent && template.template) {
            ...
            result.foundWords.forEach(({ word }) => {
              ...
              try {
                ...
                newContent = newContent.replace(searchRegex, replaceText);
              } catch (error) {
                console.warn(`Invalid regex for word: ${word}`, error);
              }
            });
          ...
          if (findReplaceOptions.searchInSubject && template.subject) {
            ...
            result.foundWords.forEach(({ word }) => {
              ...
              try {
                ...
                newSubject = newSubject.replace(searchRegex, replaceText);
              } catch (error) {
                console.warn(`Invalid regex for word: ${word}`, error);
              }
            });
  ...
  const handleFindTextChange = useCallback(
    (text) => {
      ...
      const filteredTemplates = templates.filter((template) => {
        ...
        // Search in subject - FIXED: Remove redundant includes check
        if (findReplaceOptions.searchInSubject && template.subject) {
          let subjectMatches = 0;
          if (searchRegex) {
            const matches = template.subject.match(searchRegex);
      ...
      const results = filteredTemplates.map((template) => {
        ...
        // Count subject matches - FIXED: Remove redundant includes check
        if (findReplaceOptions.searchInSubject && template.subject) {
          if (searchRegex) {
            const matches = template.subject.match(searchRegex);
            subjectMatches = matches ? matches.length : 0;
  ...
  const handleSendTestEmail = useCallback(
    async (templateId) => {
      ...
      try {
        ...
        const response = await fetch("https://honhaarjawan.pk/api/sendMail", {
          method: "POST",
          headers: {
  ...
  // Remove automatic history fetching when template is selected
  useEffect(() => {
    if (selectedTemplateId) {
      ...
      if (template) {
        ...
        setIsCreating(false);
        setHasUnsavedChanges(false);
        // No longer automatically fetch history here
      }
    } else {
      setIsEditing(false);
    }
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-gradient-to-r from-primary to-primary/90 text-white rounded-lg flex items-center gap-2 hover:from-primary/90 hover:to-primary transition-all text-sm"
                    >
